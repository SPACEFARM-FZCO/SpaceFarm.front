      ...
      <div class="zone-chart__header">
        ...
        <div class="zone-chart__header-item zone-chart__header-item--mob">
          <div>
            <h1>Temperature</h1>
            <div v-if="zoneData">
              {{ zoneData.temp_solution.value }} С · Today
            </div>
          </div>
        </div>
        ...
        <div class="zone-chart__header-item zone-chart__header-item--desk">
          <div>
            <h1>C02</h1>
            <div v-if="zoneData">{{ zoneData.c02.value }} · Today</div>
          </div>
        </div>
      </div>
      <client-only>
...
export default {
  ...
  methods: {
    getZoneData(filter = "week", type = "day") {
      this.$axios
        .$get(
          `/winrenboard/c02?number_zone=${this.$route.params.id}&filter=${filter}&type=${type}`
        )
        .then((res) => {
          this.$nextTick(() => {
            this.zoneData = res;
